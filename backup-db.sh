...
/usr/bin/mysqldump --no-tablespaces -u fafarasa -pfafarasa fafarasa > "$BACKUP_DIR/$BACKUP_FILE"
...
if [ $? -eq 0 ]; then
    ...
    /usr/bin/gzip "$BACKUP_DIR/$BACKUP_FILE"
    echo "Backup compressed: $BACKUP_DIR/$BACKUP_FILE.gz"
    
    # Hapus backup lama (lebih dari 7 hari)
    /usr/bin/find $BACKUP_DIR -name "fafarasa_backup_*.sql.gz" -mtime +7 -delete
